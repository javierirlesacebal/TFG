...
unsigned char apply_gate_to_master_state(struct master_state_vector *master_state, struct master_state_vector *new_master_state, int num_targets, unsigned int *targets, unsigned int num_controls,unsigned int *controls, unsigned int num_anticontrols, unsigned int *anticontrols) {
	...
	normalize_master_state(new_master_state);
	...
	for (lut_index = 0; lut_index < gate_lsb_size; ++lut_index) {
		...
		for (target = 0; target < num_targets; target++) {
			if (targets[target] < lsb_bitsize) {
				reg_index |= (((lut_index >> target_index) & NATURAL_ONE) << targets[target]);
				target_index++;
			}
		}
	...
	for (local_index = 0; local_index < block_size; local_index++) {
		...
		if (((global_index & control_mask) != control_mask || (global_index & anticontrol_mask) != 0)) {
			...
			for (local_gate_permutation = 0; local_gate_permutation < gate_lsb_size; local_gate_permutation++) {
				...
				gate_value = gate->matrix[row][local_gate_permutation];
...
unsigned char slave_state_join(struct slave_state_vector **local_state_vectors) {
	...
	struct slave_state_vector *result, *main, *secondary;
	int parameters1[6];
	...
	result = local_state_vectors[result_index];
	main = local_state_vectors[main_index];
	secondary = local_state_vectors[secondary_index];
	...
	if (slave_id < main_num_slaves){
		for(i = 0; i < result->num_slaves; i++){
			communication_index = slave_id * mtrsr + i;
			if (communication_index == slave_id){
				join_process_self_main(result, main, main_slave_size);
			} else if ((communication_index / mtrsr) == slave_id) {
				join_send_main(main, communication_index, mtrsr, num_main_chunks, main_chunk_size);
			}
		}
	}
	...
	if (slave_id > 0 && mtrsr != 1){
		join_recive_main(result, recived_main_chunk, main_chunk_size, main_slave_size, (slave_id / mtrsr), num_main_chunks);
	}
	
	// ENVIA, RECIBE Y PROCESA SECONDARY VECTOR
	for(i = 0; i < result->num_slaves; i++){
		communication_index = (i - slave_id + result->num_slaves) % result->num_slaves;
		value = (secondary_slave_size * secondary_num_slaves) >> log2_64((uint64_t) result->assigned_elements);
		if (value == 0){
			recive = communication_index < secondary_num_slaves;
			send = slave_id < secondary_num_slaves;
		} else {
			recive = (slave_id % value) == ((communication_index * secondary_slave_size)/result->assigned_elements);
			send = (communication_index % value) == ((slave_id * secondary_slave_size)/result->assigned_elements);
		}
		if (slave_id == communication_index && slave_id < secondary_num_slaves){
			join_process_self_secondary(result, secondary);
		}
		if (send && slave_id < communication_index && slave_id != communication_index) {
			join_send_secondary(secondary, communication_index, num_secondary_chunks, secondary_chunk_size, secondary_num_slaves);
		}
		if (recive && slave_id != communication_index){
			join_recive_secondary(result, recived_secondary_chunk, communication_index, num_secondary_chunks, secondary_chunk_size, secondary_slave_size, secondary_size, secondary_num_slaves);
		}
		if (send && slave_id >= communication_index && slave_id != communication_index) {
			join_send_secondary(secondary, communication_index, num_secondary_chunks, secondary_chunk_size, secondary_num_slaves);
		}
