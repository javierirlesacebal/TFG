...
static const size_t COMPLEX_ARRAY_SIZE = 268435456;
static const size_t NATURAL_ARRAY_SIZE = 536870912;
...
COMPLEX_TYPE fix_value(COMPLEX_TYPE a, REAL_TYPE min_r, REAL_TYPE min_i, REAL_TYPE max_r, REAL_TYPE max_i);

unsigned int log2_64(uint64_t value);


static const size_t MIN_SV_ELEMENTS = 3;
#define SLAVE_MAX_SV 64

#define SLAVE_COMMUNICATION_CHUNK_SIZE 16384
#define SLAVE_COLLAPSE_CHUNK_SIZE 16384
...
#define SAFE_MALLOC(var, size, type, err_var, err_val, label) \
    do { \
        (var) = MALLOC_TYPE((size), type); \
        if ((var) == NULL) { \
            (err_var) = (err_val); \
            goto label; \
        } \
...
#define SAFE_CALLOC(var, size, type, err_var, err_val, label) \
    do { \
        (var) = CALLOC_TYPE((size), type); \
        if ((var) == NULL) { \
            (err_var) = (err_val); \
            goto label; \
        } \
...
#define SAFE_MALLOC_CONDITIONAL(var, size, type, err_var, err_val, label, cond) \
    do { \
        if (cond) { \
            (var) = MALLOC_TYPE((size), type); \
            if ((var) == NULL) { \
...
#define MPI_SEND_TO_SLAVES(state, data_ptr, data_count, mpi_type) \
    do { \
        for (int _i = 0; _i < (state)->num_slaves; _i++) { \
            MPI_Send((data_ptr), (data_count), (mpi_type), (_i) + 1, 0, MPI_COMM_WORLD); \
        } \
    } while (0)

#define MPI_NOTIFY_READY(response, cleanup_label)                             \
    do {                                                                   \
        (response) = 1;                                                    \
...
#define MPI_SEND_ABORT(response)                                       \
    do {                                                                   \
        (response) = 0;                                                    \
        MPI_Send(&(response), 1, MPI_INT, 0, 0, MPI_COMM_WORLD);\
        MPI_Recv(&(response), 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);\
...
#define MPI_NOTIFY_FINISHED(response)                                     \
    do {                                                                  \
        (response) = 1;                                                   \
        MPI_Send(&(response), 1, MPI_INT, 0, 0, MPI_COMM_WORLD);          \
    } while (0)
